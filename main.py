...
env = UnityEnvironment(file_name="envs/Window_build/UnityEnvironment.exe", seed=episode_id, side_channels=[], worker_id=worker_id, )
env.reset()
...
while True:
    ...
    model = ActorCritic()
    model_state_dict = torch.load('alg.pt')
    model.load_state_dict(model_state_dict)
    model.eval()

    obs1 = torch.Tensor(obs1).unsqueeze(0)
    with torch.no_grad():
        action_probs = model(obs1)
        action_distribution = Categorical(action_probs)
        action1 = action_distribution.sample().item()
...
np.savez_compressed(f"compressed2/{episode_id}_obs0.npy", np.array(obs_agent0))

#np.save('episodes/{}_obs1.npy'.format(episode_id), np.array(obs_agent1))
np.savez_compressed(f"compressed2/{episode_id}_obs1.npy", np.array(obs_agent1))

#np.save('episodes/{}_action0.npy'.format(episode_id), np.array(action_agent0))
np.savez_compressed(f"compressed2/{episode_id}_action0.npy", np.array(action_agent0))

#np.save('episodes/{}_action1.npy'.format(episode_id), np.array(action_agent1))
np.savez_compressed(f"compressed2/{episode_id}_action1.npy", np.array(action_agent1))

#np.save('episodes/{}_rewards.npy'.format(episode_id), np.array(rewards))
np.savez_compressed(f"compressed2/{episode_id}_rewards.npy", np.array(rewards))
